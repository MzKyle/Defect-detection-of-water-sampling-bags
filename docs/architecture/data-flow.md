...
sequenceDiagram
    ...
    Camera->>Runtime: 新图片创建
    Runtime->>Queue: build FramePacket
    Queue->>Runtime: worker 取包
    Runtime->>Runtime: 等待文件稳定 / cooldown
    Runtime->>Pipeline: process_packet(packet)
    Pipeline->>Pipeline: 备份原图
    Pipeline->>Detector: Stage 1 整图检测
    alt Stage 1 未命中
...
| 阶段 | 输入 | 输出 |
| --- | --- | --- |
| 接入 | 图片路径、相机配置 | `FramePacket` |
| Stage 1 | `FramePacket.source_path` | `PerceptionResult(stage1)` |

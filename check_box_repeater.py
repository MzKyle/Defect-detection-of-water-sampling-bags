...
def save_boxes(boxes, filepath=save_path):
    ...
    for box in boxes:
        ...
        new_lines.append(' '.join([f"{v:.2f}" for v in norm]) + '\n')
...
def check_box_repeat(boxes_dict_list, iou_thresh=0.5, verbose=True):
    ...
    if any_match:
        if verbose:
            print("→ Result repeated (1)")
        return 1
    else:
        if verbose:
            print("→ New result, saving... (2)")

...
```text
工位线程：相机输入 -> 是否有袋快速检测 -> burst 抓拍锁存 -> PLC 拨杆动作 -> BagID 样本组装
推理线程：完整袋体样本 -> 缺陷检测 -> 袋级状态机 -> BagID 顺序重排 -> 末端分拣 PLC
```
...
2. 先运行轻量级水样袋 presence 检测，判断当前视野是否有袋。
...
8. 四个 defect worker 可以乱序完成推理，但最终结果必须进入 `SortReorderBuffer`。
9. 末端分拣按物理进入顺序释放 `bag_id`，再驱动 `end_sorter:route_to_ok_bin` 或 `end_sorter:route_to_ng_bin`。
10. 如果采图齐套或推理结果超时，默认输出 NG，符合 fail-safe 策略。
11. 将时序、PLC 反馈、状态轨迹、presence 结果和缺陷结果写入 JSONL。
```
...
| 路径 | 作用 |
| --- | --- |
| `camera_driver/` | 相机传感、多光源 burst 采图和图像组包 |
| `PLC_driver/` | PLC 光源切换事件、拨杆动作和末端分拣动作 |
| `detect_orchestrator/` | 主流程编排、presence gate、异步缺陷 worker 池、结果融合 |
...
CMake 当前暴露三个物理包，并且公共头文件入口与包名一致：

...
def _build_parser() -> argparse.ArgumentParser:
    ...
    seed_parser.add_argument("--output-root", default="demo_data", help="directory containing camera1 and camera2")
    seed_parser.add_argument("--clean", action="store_true", help="remove the output directory before generating")

    inspect_parser = subparsers.add_parser("inspect", help="run the pipeline once on a single image")
    inspect_parser.add_argument("--config", help="path to YAML config file")
    inspect_parser.add_argument("--camera-id", type=int, default=1, help="camera id defined in config")
    ...
    replay_parser = subparsers.add_parser("replay", help="replay a directory of historical images through the pipeline")
    replay_parser.add_argument("--config", help="path to YAML config file")
    replay_parser.add_argument("--source-root", default="demo_data", help="root directory containing camera1/camera2")
    replay_parser.add_argument("--interval-ms", type=int, default=0, help="sleep interval between replayed frames")
    ...
    inject_parser = subparsers.add_parser("inject-faults", help="run offline fault injection scenarios")
    ...
    inject_parser.add_argument("--clean", action="store_true", help="remove the output directory before running")
...
def _run_inspect(config: str | None, camera_id: int, image_path: str, reset_history: bool) -> None:
    ...
    if camera_id not in settings.camera_map:
        raise SystemExit(f"Unknown camera_id={camera_id}. Available cameras: {sorted(settings.camera_map)}")
...
def _run_replay(config: str | None, source_root: str, interval_ms: int, limit: int | None, reset_history: bool) -> None:
    from .main import build_pipeline_components, configure_logging
...
def _run_fault_injections(config: str | None, scenario: str, output_root: str, clean: bool) -> None:
    ...
    payload = run_fault_injections(config_path=config, scenario=scenario, output_root=output_root)
    print(json.dumps(payload, ensure_ascii=False, indent=2))
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "replay":
        _run_replay(args.config, args.source_root, args.interval_ms, args.limit, args.reset_history)
        return
